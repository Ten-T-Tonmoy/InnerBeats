export const response = {
  Sad: [
    {
      advice: {
        heading: "Feeling low?",
        content: "It's okay to cry. Talk to someone you trust.",
      },
      quote: {
        content: "Every storm runs out of rain.",
        writer: "folklore",
      },
      bgCode:
        "https://images.pexels.com/photos/1191710/pexels-photo-1191710.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2",
      songUrl: "https://example.com/sad-song-1",
      todo: ["Take a walk", "Journal your thoughts"],
    },
    {
      advice: {
        heading: "This too shall pass",
        content: "Allow yourself to feel, but remember healing takes time.",
      },
      quote: {
        content: "The wound is the place where the Light enters you.",
        writer: "Rumi",
      },
      bgCode: "bg-gray-700",
      songUrl: "https://example.com/sad-song-2",
      todo: ["Call a friend", "Listen to calming music"],
    },
    {
      advice: {
        heading: "Gentle with yourself",
        content: "Sadness is not weakness, it's your heart processing change.",
      },
      quote: {
        content: "There is a sacredness in tears.",
        writer: "Washington Irving",
      },
      bgCode: "bg-blue-800",
      songUrl: "https://example.com/sad-song-3",
      todo: ["Write down 3 things you're grateful for", "Take a warm bath"],
    },
    {
      advice: {
        heading: "You're not alone",
        content: "Reach out. Sometimes sharing the burden makes it lighter.",
      },
      quote: {
        content: "Grief is the price we pay for love.",
        writer: "Queen Elizabeth II",
      },
      bgCode: "bg-indigo-700",
      songUrl: "https://example.com/sad-song-4",
      todo: ["Watch a comfort movie", "Hug a pet or loved one"],
    },
    {
      advice: {
        heading: "Tomorrow is a new day",
        content: "Rest tonight. Tomorrow brings new possibilities.",
      },
      quote: {
        content: "Night is always darkest before the dawn.",
        writer: "Thomas Fuller",
      },
      bgCode: "bg-slate-800",
      songUrl: "https://example.com/sad-song-5",
      todo: ["Get enough sleep", "Plan something small to look forward to"],
    },
    {
      advice: {
        heading: "Honor your feelings",
        content:
          "Your emotions are valid. Feel them fully, then let them flow.",
      },
      quote: {
        content: "The only way out is through.",
        writer: "Robert Frost",
      },
      bgCode: "bg-gray-600",
      songUrl: "https://example.com/sad-song-6",
      todo: ["Practice deep breathing", "Create something with your hands"],
    },
    {
      advice: {
        heading: "Small steps matter",
        content: "You don't have to climb the whole mountain today.",
      },
      quote: {
        content:
          "It is during our darkest moments that we must focus to see the light.",
        writer: "Aristotle",
      },
      bgCode: "bg-stone-700",
      songUrl: "https://example.com/sad-song-7",
      todo: ["Do one small act of self-care", "Step outside for fresh air"],
    },
  ],

  Angry: [
    {
      advice: {
        heading: "Breathe in, breathe out",
        content: "Try to focus on what you can control.",
      },
      quote: {
        content: "Holding onto anger is like drinking poison.",
        writer: "Marcus Aurelius",
      },
      bgCode: "bg-red-600",
      songUrl: "https://example.com/angry-song-1",
      todo: ["Punch a pillow", "Do some pushups"],
    },
    {
      advice: {
        heading: "Channel that fire",
        content: "Use this energy for positive change instead of destruction.",
      },
      quote: {
        content:
          "Anger is an acid that can do more harm to the vessel than to anything on which it is poured.",
        writer: "Mark Twain",
      },
      bgCode: "bg-orange-600",
      songUrl: "https://example.com/angry-song-2",
      todo: ["Go for a run", "Write an angry letter (don't send it)"],
    },
    {
      advice: {
        heading: "Count to ten",
        content:
          "Take a pause before reacting. Your future self will thank you.",
      },
      quote: {
        content:
          "For every minute you remain angry, you give up sixty seconds of peace of mind.",
        writer: "Ralph Waldo Emerson",
      },
      bgCode: "bg-red-700",
      songUrl: "https://example.com/angry-song-3",
      todo: ["Take 10 slow deep breaths", "Clench and release your fists"],
    },
    {
      advice: {
        heading: "Find the root cause",
        content:
          "What's really bothering you? Address the source, not just the symptom.",
      },
      quote: {
        content:
          "Anger is often what pain looks like when it shows itself in public.",
        writer: "Krista Tippett",
      },
      bgCode: "bg-amber-700",
      songUrl: "https://example.com/angry-song-4",
      todo: ["Identify what triggered you", "Talk to someone you trust"],
    },
    {
      advice: {
        heading: "Physical release",
        content: "Your body is holding tension. Move it out of your system.",
      },
      quote: {
        content:
          "Speak when you are angry and you will make the best speech you will ever regret.",
        writer: "Ambrose Bierce",
      },
      bgCode: "bg-red-800",
      songUrl: "https://example.com/angry-song-5",
      todo: ["Do jumping jacks", "Scream into a pillow"],
    },
    {
      advice: {
        heading: "Choose your battles",
        content:
          "Not every hill is worth dying on. Save your energy for what matters.",
      },
      quote: {
        content: "The best fighter is never angry.",
        writer: "Lao Tzu",
      },
      bgCode: "bg-rose-700",
      songUrl: "https://example.com/angry-song-6",
      todo: [
        "List what you can and can't control",
        "Practice progressive muscle relaxation",
      ],
    },
    {
      advice: {
        heading: "Cool down time",
        content: "Step away from the situation. Distance brings clarity.",
      },
      quote: {
        content: "When angry, count to four; when very angry, swear.",
        writer: "Mark Twain",
      },
      bgCode: "bg-red-500",
      songUrl: "https://example.com/angry-song-7",
      todo: ["Take a cold shower", "Go somewhere quiet for 15 minutes"],
    },
    {
      advice: {
        heading: "Transform the energy",
        content: "Anger can be fuel for positive action and necessary change.",
      },
      quote: {
        content: "Anger is a wind which blows out the lamp of the mind.",
        writer: "Robert Green Ingersoll",
      },
      bgCode: "bg-orange-700",
      songUrl: "https://example.com/angry-song-8",
      todo: ["Clean or organize something", "Do a high-intensity workout"],
    },
  ],

  Lost: [
    {
      advice: {
        heading: "It's okay not to know",
        content:
          "Being lost is often the first step to finding something better.",
      },
      quote: {
        content: "Not all those who wander are lost.",
        writer: "J.R.R. Tolkien",
      },
      bgCode: "bg-purple-600",
      songUrl: "https://example.com/lost-song-1",
      todo: ["Write down your values", "Take a different route home"],
    },
    {
      advice: {
        heading: "One step at a time",
        content: "You don't need to see the whole path, just the next step.",
      },
      quote: {
        content: "The cave you fear to enter holds the treasure you seek.",
        writer: "Joseph Campbell",
      },
      bgCode:
        "https://images.pexels.com/photos/3586966/pexels-photo-3586966.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2",
      songUrl: "https://www.youtube.com/watch?v=xFYQQPAOz7Y",
      todo: ["Try something new today", "Ask someone about their journey"],
    },
    {
      advice: {
        heading: "Trust the process",
        content:
          "Sometimes we need to lose ourselves to find who we really are.",
      },
      quote: {
        content: "The journey of a thousand miles begins with one step.",
        writer: "Lao Tzu",
      },
      bgCode: "bg-violet-700",
      songUrl: "https://example.com/lost-song-3",
      todo: ["Meditate for 10 minutes", "Read about someone who inspires you"],
    },
    {
      advice: {
        heading: "Explore possibilities",
        content:
          "Being lost means you're open to new directions and opportunities.",
      },
      quote: {
        content: "Adventure is worthwhile in itself.",
        writer: "Amelia Earhart",
      },
      bgCode:
        "https://images.pexels.com/photos/3586966/pexels-photo-3586966.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2",
      songUrl: "https://www.youtube.com/watch?v=xFYQQPAOz7Y",
      todo: [
        "Make a list of things you're curious about",
        "Visit somewhere you've never been",
      ],
    },
    {
      advice: {
        heading: "Connect with your inner compass",
        content:
          "Quiet the noise around you and listen to what your heart says.",
      },
      quote: {
        content:
          "Your only obligation in any lifetime is to be true to yourself.",
        writer: "Richard Bach",
      },
      bgCode: "bg-teal-600",
      songUrl: "https://example.com/lost-song-5",
      todo: ["Journal without censoring yourself", "Spend time in nature"],
    },
    {
      advice: {
        heading: "Embrace uncertainty",
        content:
          "Uncertainty is where possibilities live. You're exactly where you need to be.",
      },
      quote: {
        content: "Life is either a daring adventure or nothing at all.",
        writer: "Helen Keller",
      },
      bgCode: "bg-cyan-700",
      songUrl: "https://example.com/lost-song-6",
      todo: ["Talk to a mentor or wise friend", "Create a vision board"],
    },
    {
      advice: {
        heading: "Look for clues in your past",
        content:
          "What made you feel most alive? Those are breadcrumbs to follow.",
      },
      quote: {
        content: "The privilege of a lifetime is to become who you truly are.",
        writer: "Carl Jung",
      },
      bgCode: "bg-sky-600",
      songUrl: "https://example.com/lost-song-7",
      todo: [
        "List moments when you felt most yourself",
        "Reconnect with an old passion",
      ],
    },
    {
      advice: {
        heading: "Be patient with yourself",
        content:
          "Finding your way takes time. Every detour teaches you something.",
      },
      quote: {
        content:
          "The real voyage of discovery consists not in seeking new landscapes, but in having new eyes.",
        writer: "Marcel Proust",
      },
      bgCode: "bg-purple-700",
      songUrl: "https://example.com/lost-song-8",
      todo: ["Practice self-compassion", "Start a small experiment or project"],
    },
  ],

  Scared: [
    {
      advice: {
        heading: "Feel the fear and do it anyway",
        content: "Courage isn't the absence of fear, it's acting despite it.",
      },
      quote: {
        content:
          "You are braver than you believe, stronger than you seem, and smarter than you think.",
        writer: "A.A. Milne",
      },
      bgCode: "bg-yellow-600",
      songUrl: "https://example.com/scared-song-1",
      todo: ["Name what you're afraid of", "Take one small brave action"],
    },
    {
      advice: {
        heading: "Ground yourself in the present",
        content:
          "Fear lives in the future. Come back to this moment. You're safe right now.",
      },
      quote: {
        content: "Anxiety is the dizziness of freedom.",
        writer: "Søren Kierkegaard",
      },
      bgCode: "bg-amber-600",
      songUrl: "https://example.com/scared-song-2",
      todo: [
        "Practice 5-4-3-2-1 grounding technique",
        "Hold something comforting",
      ],
    },
    {
      advice: {
        heading: "Break it down",
        content: "Big fears become manageable when broken into smaller pieces.",
      },
      quote: {
        content: "The only thing we have to fear is fear itself.",
        writer: "Franklin D. Roosevelt",
      },
      bgCode: "bg-orange-500",
      songUrl: "https://example.com/scared-song-3",
      todo: ["Write down your specific worries", "Create a simple action plan"],
    },
    {
      advice: {
        heading: "Remember your strength",
        content:
          "You've overcome challenges before. You have that same strength now.",
      },
      quote: {
        content: "Fear is temporary. Regret is forever.",
        writer: "Unknown",
      },
      bgCode: "bg-yellow-700",
      songUrl: "https://example.com/scared-song-4",
      todo: [
        "List past challenges you've overcome",
        "Call someone who believes in you",
      ],
    },
    {
      advice: {
        heading: "Breathe through it",
        content:
          "Your breath is your anchor. Use it to stay steady in the storm.",
      },
      quote: {
        content: "Breathe in courage, breathe out fear.",
        writer: "Theodore Roosevelt",
      },
      bgCode: "bg-lime-600",
      songUrl: "https://example.com/scared-song-5",
      todo: [
        "Practice box breathing (4-4-4-4)",
        "Progressive muscle relaxation",
      ],
    },
    {
      advice: {
        heading: "Seek support",
        content: "You don't have to face your fears alone. Reach out for help.",
      },
      quote: {
        content: "Shared joy is double joy; shared sorrow is half sorrow.",
        writer: "Swedish Proverb",
      },
      bgCode: "bg-emerald-600",
      songUrl: "https://example.com/scared-song-6",
      todo: [
        "Reach out to a trusted friend",
        "Consider professional support if needed",
      ],
    },
    {
      advice: {
        heading: "Prepare and take action",
        content: "Preparation builds confidence. Take one step forward.",
      },
      quote: {
        content: "You miss 100% of the shots you don't take.",
        writer: "Wayne Gretzky",
      },
      bgCode: "bg-teal-700",
      songUrl: "https://example.com/scared-song-7",
      todo: ["Research and prepare", "Take the smallest possible first step"],
    },
    {
      advice: {
        heading: "Reframe your thoughts",
        content:
          "What if this scary thing is actually an opportunity in disguise?",
      },
      quote: {
        content: "Everything you want is on the other side of fear.",
        writer: "Jack Canfield",
      },
      bgCode: "bg-green-600",
      songUrl: "https://example.com/scared-song-8",
      todo: [
        "Write down worst and best case scenarios",
        "Focus on what you can control",
      ],
    },
    {
      advice: {
        heading: "Trust yourself",
        content:
          "You have inner wisdom and strength. Trust your ability to handle whatever comes.",
      },
      quote: {
        content:
          "The way to develop self-confidence is to do the thing you fear.",
        writer: "William Jennings Bryan",
      },
      bgCode: "bg-yellow-800",
      songUrl: "https://example.com/scared-song-9",
      todo: ["Visualize yourself succeeding", "Practice positive self-talk"],
    },
  ],

  Happy: [
    {
      advice: {
        heading: "Savor this moment",
        content:
          "Happiness is fleeting - take time to really feel and appreciate it.",
      },
      quote: {
        content:
          "Happiness is not something ready made. It comes from your own actions.",
        writer: "Dalai Lama",
      },
      bgCode: "bg-green-500",
      songUrl: "https://example.com/happy-song-1",
      todo: [
        "Share your joy with someone",
        "Take a photo to remember this feeling",
      ],
    },
    {
      advice: {
        heading: "Spread the joy",
        content:
          "Happiness multiplies when shared. Let your light shine on others.",
      },
      quote: {
        content: "The most wasted of days is one without laughter.",
        writer: "E.E. Cummings",
      },
      bgCode: "bg-lime-500",
      songUrl: "https://example.com/happy-song-2",
      todo: ["Compliment a stranger", "Do something kind for someone"],
    },
    {
      advice: {
        heading: "Dance like nobody's watching",
        content: "Let your body express the joy your heart is feeling.",
      },
      quote: {
        content: "Life is too important to be taken seriously.",
        writer: "Oscar Wilde",
      },
      bgCode: "bg-yellow-400",
      songUrl: "https://example.com/happy-song-3",
      todo: ["Dance to your favorite song", "Sing out loud"],
    },
    {
      advice: {
        heading: "Capture the good vibes",
        content:
          "Write down what made you happy today so you can return to it later.",
      },
      quote: {
        content: "Collect moments, not things.",
        writer: "Unknown",
      },
      bgCode: "bg-emerald-400",
      songUrl: "https://example.com/happy-song-4",
      todo: ["Start a gratitude journal", "Take a mental snapshot"],
    },
    {
      advice: {
        heading: "Play and be silly",
        content:
          "Embrace your inner child. Play is the highest form of research.",
      },
      quote: {
        content:
          "We don't stop playing because we grow old; we grow old because we stop playing.",
        writer: "George Bernard Shaw",
      },
      bgCode: "bg-pink-400",
      songUrl: "https://example.com/happy-song-5",
      todo: ["Do something playful", "Laugh until your belly hurts"],
    },
    {
      advice: {
        heading: "Be present in your joy",
        content:
          "Don't worry about when this feeling will end. Just be here now.",
      },
      quote: {
        content:
          "Yesterday is history, tomorrow is a mystery, today is a gift.",
        writer: "Eleanor Roosevelt",
      },
      bgCode: "bg-rose-400",
      songUrl: "https://example.com/happy-song-6",
      todo: ["Practice mindful appreciation", "Breathe in the good feelings"],
    },
    {
      advice: {
        heading: "Create and express",
        content:
          "Channel your happiness into something creative and beautiful.",
      },
      quote: {
        content: "Creativity takes courage.",
        writer: "Henri Matisse",
      },
      bgCode: "bg-orange-400",
      songUrl: "https://example.com/happy-song-7",
      todo: ["Create something artistic", "Write a poem or story"],
    },
    {
      advice: {
        heading: "Connect with nature",
        content: "Step outside and let the natural world amplify your joy.",
      },
      quote: {
        content:
          "In every walk with nature, one receives far more than they seek.",
        writer: "John Muir",
      },
      bgCode: "bg-teal-400",
      songUrl: "https://example.com/happy-song-8",
      todo: [
        "Go outside and feel the sun",
        "Plant something or tend to plants",
      ],
    },
    {
      advice: {
        heading: "Celebrate yourself",
        content: "You deserve this happiness. Celebrate what brought you here.",
      },
      quote: {
        content: "Be happy for this moment. This moment is your life.",
        writer: "Omar Khayyam",
      },
      bgCode: "bg-cyan-400",
      songUrl: "https://example.com/happy-song-9",
      todo: [
        "Treat yourself to something nice",
        "Acknowledge your accomplishments",
      ],
    },
    {
      advice: {
        heading: "Pay it forward",
        content:
          "Use your positive energy to make someone else's day brighter.",
      },
      quote: {
        content:
          "Happiness is the only thing that multiplies when you share it.",
        writer: "Albert Schweitzer",
      },
      bgCode: "bg-green-400",
      songUrl: "https://example.com/happy-song-10",
      todo: ["Volunteer or help someone", "Leave a positive review or note"],
    },
  ],
};
